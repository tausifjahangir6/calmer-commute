...
| Quality Controls: team-approved code-quality and security checks are included | `npm run lint`, `pytest`, `pip-audit -r backend/requirements.txt`, `npm audit` | `frontend/package.json`, `backend/requirements.txt`, `.github/workflows/ci.yml`, `docs/testing/security-scans/` | CI quality checks |
| Security and failure assessment: a documented threat/failure register exists and is mapped to controls, tests, and artifacts | `docs/testing/VERTICAL_SLICE_SECURITY_ASSESSMENT.md`, privacy retention rule, and retained scan reports | `docs/testing/VERTICAL_SLICE_SECURITY_ASSESSMENT.md`, `docs/testing/PRIVACY_RETENTION.md`, `docs/testing/security-scans/`, `.github/workflows/ci.yml` | Manual review + CI evidence |
